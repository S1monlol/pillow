...
class JSRewriter {
    constructor(ctx) {
        ...
        this.map = [
            ...
            },
            {
                ...
                handler: (node, parent) => {
                    node.source = this.createCallExpression(this.createMemberExpression(this.createMemberExpression(this.createIdentifier('$c'), this.createIdentifier('url')), this.createIdentifier('wrap')), [ 
                        node.source,
                        this.createMemberExpression(this.createIdentifier('$c'), this.createIdentifier('meta')),
                    ]);
                },
            },  
        ];
    ...
    createHead(url) {
        return `
        if (!self.$c && self.importScripts) {
            importScripts(location.origin + '${this.ctx.prefix}index.js');
            self.$c = new Corrosion({ url: '${url}', codec: '${this.ctx.config.codec || 'plain'}', serviceWorker: true,  window: self, prefix: '${this.ctx.prefix || '/service/'}', ws: ${this.ctx.config.ws || true}, cookies: ${this.ctx.config.cookies || false}, title: '${this.ctx.config.title}', }); $c.init();
        };\n`;
    };
    iterate(ast, handler) {
