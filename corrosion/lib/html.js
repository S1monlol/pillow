...
class HTMLRewriter {
    ...
    process(source, config = {}) {
        ...
        iterate(ast, node => {
            ...
            node.attrs.forEach(attr => {
                ...
                switch(handler) {
                    case 'url':
                        node.setAttribute(`c-${attr.name}`, attr.value);
    ...
    source(processed, config = {}) {
        ...
        iterate(ast, node => {
            if (!node.tagName) return;
            node.attrs.forEach(attr => {
                if (node.hasAttribute(`c-${attr.name}`)) { 
                    attr.value = node.getAttribute(`c-${attr.name}`);
                    node.removeAttribute(`c-${attr.name}`)
                };
            });
        });
        return parse5.serialize(ast);
        ...
        return [
            ...
            { 
                ...
                childNodes: [
                    {
                        nodeName: '#text',
                        value: `window.$c = new Corrosion({ window, codec: '${this.ctx.config.codec || 'plain'}',  prefix: '${this.ctx.prefix}', ws: ${this.ctx.config.ws}, cookie: ${this.ctx.config.cookie}, title: ${typeof this.ctx.config.title == 'boolean' ? this.ctx.config.title : '\'' + this.ctx.config.title + '\''}, }); $c.init(); document.currentScript.remove();`
                    },
                ], 
                attrs: [],
            }
